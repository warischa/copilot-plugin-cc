...
- **[x] Live-binary coverage of the companion dispatcher (wave 5, 2026-05-29).** The "needs the live `copilot` binary" gap is now covered: `tests/integration.test.mjs` gained gated tests for `setup`, `review`, `adversarial-review` (incl. the read-only invariant), and the full background `task-worker` lifecycle (`task --background` → `status --wait` → `result`), on top of the original foreground `task` test. `copilot-companion.mjs` stays ~35% line in the **default ungated** run (the integration tests skip) but rises to **~80.9% line / ~87.1% funcs** with `COPILOT_INTEGRATION=1`. Remaining intentional ceiling: `process.mjs` `terminateProcessTree` (kills process trees — out of scope for hermetic unit tests).
