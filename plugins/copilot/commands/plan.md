...
argument-hint: "[--wait|--background] [--model <model>] [--effort <none|low|medium|high|xhigh|max>] [--share[=<path>]|--share-path <path>] [--share-gist] [--mcp-tool <names>] [--mcp-config <json|@file>] [--allow-tool <pats>] [--allow-url <urls>] [--deny-url <urls>] [--allow-remote] [--allow-ask-user] [what to plan]"
